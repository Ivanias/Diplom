...
image,fon,x,y,w,h=func.truckDetection(image,fon,x,y,w,h,kheight,kwidth)
...
imgBlank = np.zeros((heightImg, widthImg, 3), np.uint8)
imgThreshold = func.imagePreparation(img)
...
if biggest.size != 0:
    ...
    newfon=func.domatrix(fon,matrix,widthImg,heightImg)

    # Обработка повёрнутого изображения для того чтобы нахождения контуров стало возможным
    imgThreshold1 = func.imagePreparation(newfon)
    ...
    if biggest1.size != 0:
        ...
        plan=func.coordinatesOnThePlan(biggest1,newfon,plan)
